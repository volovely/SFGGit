...
class ClaudeClient: ObservableObject {
    ...
    func generatePRInfo(from diff: String) async -> PRInfo? {
        guard !apiKey.isEmpty else {
            print("Claude API key not configured")
            return nil
        }
        ...
        let systemPrompt = """
        You are a GitHub Pull Request assistant. Based on the provided git diff, generate a PR title and summary in JSON format.
        ...
        - Keep it detailed but well-structured

        Return ONLY valid JSON in this exact format:
        {
          "title": "PR title here",
          "summary": "Detailed PR summary with markdown formatting"
        }
        """
        ...
        let requestBody: [String: Any] = [
            ...
            "messages": [
                [
                    "role": "user",
                    "content": "Generate a PR title and summary for this diff:\n\n\(diff)"
                ]
            ]
        ]
        ...
        do {
            ...
            guard let jsonData = jsonString.data(using: .utf8),
                  let prJson = try JSONSerialization.jsonObject(with: jsonData) as? [String: Any],
                  let title = prJson["title"] as? String,
                  let summary = prJson["summary"] as? String else {
                print("Failed to parse PR JSON: \(jsonString)")
                return nil
            }

            return PRInfo(title: title, summary: summary)

        } catch {
            print("Failed to generate PR info: \(error)")
            return nil
        }
    }
}
